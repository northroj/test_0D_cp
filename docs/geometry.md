...
Defines a cartesian mesh for the geometry (just one large x,y,z grid). Currently, all external boundaries are vacuum.
...
| `fill` | int | Fill every cell with a given `mat_id` |
| `block` | 7 ints | Given indices, fills a block with a material [-x_idx, +x_idx, -y_idx, +y_idx, -z_idx, +z_idx, mat_id] |
