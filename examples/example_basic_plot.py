...
with h5py.File("example_basic.h5","r") as f: 
    ...
    energy_bins = np.array(tally1["energy_bins"][:])
    time_bins = np.array(tally1["time_bins"][:])
    tally2 = f["tallies"]["user_tallies"]["test_tally_2"]
    average_particle_energy = np.array(tally2["counts"][:])
    average_p_e_time = np.array(tally2["time_bins"][:])
    ...
    standard_time_bins = np.array(tallystandard["ion_density_time"]["time_bins"][:])
...
plot_avg_particle_energy = average_particle_energy[0,:,0]
plt.figure(200)
