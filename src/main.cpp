...
static std::vector<double> make_bins(double lo, double hi, int divs, bool logspace) {
    ...
    // log spacing: geometric progression over [lo, hi], both > 0
    if (lo <= 0.0 || hi <= 0.0)
    ...
    // avoid last-step floating error
    edges.back() = hi;
    return edges;
}
...
static bool h5_write_ndarray_counts(hid_t parent, const char* name, const NDArray& arr) {
    // Expect dims.size()==3 for {S,T,E}; generalizes if you later add more axes.
...
// Simple helper: write all tallies under parent_group.
